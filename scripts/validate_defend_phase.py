...
def main():
    ...
    # 连续分离 20 个 physics steps 后才认为接触彻底结束
    separation_confirm_steps = 20
        ...
        while not viewer.closed and time.monotonic() < deadline:
            ...
            for _ in range(steps):
                ...
                if defend_phase == "DEFEND":
                    ...
                    if center_distance > release_threshold:
                        separated_steps += 1
                    else:
                        separated_steps = 0

                    if separated_steps >= separation_confirm_steps:
                        ...
                        print("[POST-CONTACT] PUCK SEPARATED")
                        print(
                            f"post puck: "
                            f"x={post_pos[0]:+.4f}, "
                            f"y={post_pos[1]:+.4f}, "
